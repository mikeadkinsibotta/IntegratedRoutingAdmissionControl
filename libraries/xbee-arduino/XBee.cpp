...
Tx64Request::Tx64Request(const XBeeAddress64 &addr64, const uint8_t option, uint8_t *data, const uint8_t dataLength, const uint8_t frameId) : PayloadRequest(TX_64_REQUEST, frameId, data, dataLength) {
	_addr64 = addr64;
	_option = option;
}

Tx64Request::Tx64Request(const XBeeAddress64 &addr64, uint8_t *data, const uint8_t dataLength) : PayloadRequest(TX_64_REQUEST, DEFAULT_FRAME_ID, data, dataLength) {
	_addr64 = addr64;
	_option = ACK_OPTION;
}
